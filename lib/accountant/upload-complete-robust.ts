/**
 * השלמת העלאת מסמך מצד רואה חשבון (ניסויים חוזרים לאחר PUT).
 */
...
/** POST complete-upload (רואה חשבון) עם ניסויים חוזרים */
export async function completeUploadRobustAccountant(
  documentId: string,
  ...
  for (let i = 0; i < COMPLETE_UPLOAD_DELAYS_MS.length; i += 1) {
    ...
    try {
      res = await fetch(
        `/api/accountants/me/documents/${documentId}/complete-upload`,
        {
          method: "POST",
      ...
      lastMessage =
        "שגיאת רשת בזמן השלמת ההעלאה. יש לנסות שוב בעוד רגע.";
      continue;
    }
