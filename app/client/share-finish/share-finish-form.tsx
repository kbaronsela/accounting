...
export function ShareFinishForm({
  stagingId,
  clients,
  suggestedFileName,
}: Props) {
  ...
  async function onSubmit(e: React.FormEvent) {
    ...
    setBusy(true);
    try {
      const res = await fetch("/api/client/share-finish", {
        method: "POST",
      ...
      router.push(`/client/documents/${docId}`);
    } catch {
  ...
  return (
    ...
    >
      ...
      <p className="text-xs text-zinc-600">
        {suggestedFileName ? (
          <>
            סוג מהשיתוף: קובץ <span dir="ltr">{suggestedFileName}</span>.
          </>
        ) : (
          <>הקובץ הועבר בשיתוף — יש להשמור לפי הבחירה למעלה.</>
        )}
      </p>
      {error ? (
