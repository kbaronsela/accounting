...
/** השלמת או ביטול טיוטת העלאה — מסלול API של רואה חשבון */
export function AccountantDraftUploadResumeButton({
  documentId,
  onDone,
}: {
  documentId: string;
  ...
  async function onResume() {
    setMessage(null);
    setBusy(true);
    const r = await completeUploadRobustAccountant(documentId);
    setBusy(false);
    if (!r.ok) {
