...
type Props = {
  clients: AccountantUploadClientOption[];
  /** אחרי העלאה מוצלחת — מתאים למשל להרחבת סינון ולרענון רשימה */
  onSuccessfulUpload?: (clientId: string) => void;
};

export function AccountantUploadSection({
  clients,
  onSuccessfulUpload,
}: Props) {
    ...
    return (
      <section className="rounded-lg border border-zinc-200 bg-white p-4 shadow-sm sm:p-6">
        ...
        <p className="mt-2 text-sm text-zinc-600">
          כדי להעלות מסמך נדרש לקוח במערכת. יוצרים לקוח מתפריט ניהול הלקוחות.
        </p>
      </section>
    );
  }
  ...
  async function onSubmit(e: React.FormEvent) {
    ...
    setMessage(null);
    setError(null);
    const chosenClientId = clientId;
    if (!chosenClientId) {
      setError("נא לבחור לקוח עבורו מעלים את המסמך.");
      return;
    }
    if (!file) {
    ...
    setPending(true);
    try {
      const createRes = await fetch("/api/accountants/me/documents/uploads", {
        method: "POST",
      ...
      const putPath = `/api/accountants/me/documents/${documentId}/upload`;
      ...
      const done = await completeUploadRobustAccountant(documentId);
      if (!done.ok) {
        setError(
          done.errorMessage ??
            "השלמת ההעלאה נכשלה. אפשר לנסות «להשלים העלאה» מהרשימה אם הקובץ כבר בשרת.",
        );
        setPending(false);
        return;
      }
      ...
      setMessage(
        "הקובץ הועלה והמסמך נכנס לעיבוד. אפשר לעקוב אחרי הסטטוס ברשימה למטה.",
      );
      setFile(null);
      resetAllFileInputs();
  ...
  return (
    <section className="rounded-lg border border-zinc-200 bg-white p-4 shadow-sm sm:p-6">
      ...
      <p className="mt-1 text-sm text-zinc-600">
        העלאה כרואה חשבון עבור לקוח שבחרתם — PDF או תמונה, עד 20MB למסמך.
        אחרי העיבוד הלקוח יוכל לערוך ולשלוח כרגיל.
      </p>
      <form
        ref={formRef}
      ...
      >
        ...
        <div className="hidden sm:block">
          <label
            htmlFor="acct-upload-file-desktop"
            className="mb-1 flex flex-wrap items-center gap-0 text-sm text-zinc-700"
          >
            קובץ
            <RequiredFieldMark />
        ...
        <div className="space-y-2 sm:hidden">
          ...
          <input
            ...
            id="acct-upload-file-mobile-pick"
            type="file"
            accept={FILE_ACCEPT_HINT}
            className="sr-only"
